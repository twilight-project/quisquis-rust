...
impl Shuffle {
    ...
    pub fn shuffle_argument_prove(&self){
        ...
        let (b_mat, b_dash) = create_b_b_dash(x, &self.shuffled_tau.as_row_major(), &self.pi);
        // println!("{:?}", b_mat);
    ...
    // self.multiexp_commit_prove(&b_mat, &pc_gens, &xpc_gens, &pk, rho_b);
        ...
        product::product_argument_prove(&b_mat, &pc_gens, &xpc_gens, x);
...
/// 
pub fn create_b_b_dash(x: Scalar, tau: &Vec<Scalar>, p: &Permutation) -> (Array2D<Scalar>, Array2D<Scalar>)
{
...
mod test {
    ...
    fn b_vector_test() {
       ...
       let b_reference: Vec<Scalar> = vec![Scalar::from(9u64), 
                                            Scalar::from(3u64), 
                                            Scalar::from(27u64), 
                                            Scalar::from(6561u64), 
                                            Scalar::from(2187u64), 
