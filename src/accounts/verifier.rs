...
impl<'a> Verifier<'a> {
    ...
    pub fn verify_update_ddh_verifier(g: CompressedRistretto, h: CompressedRistretto, g_dash: CompressedRistretto, h_dash: CompressedRistretto, z: Scalar, x:Scalar) -> bool{
        ...
        let g_r = Verifier::multiscalar_multiplication(&combined_scalars, &g_point).unwrap().compress();
        let h_point = vec![h, h_dash];
        let h_r = Verifier::multiscalar_multiplication(&combined_scalars, &h_point).unwrap().compress();
