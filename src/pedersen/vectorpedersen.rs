...
impl VectorPedersenGens {
    ...
    pub fn commit(&self, values: &[Scalar], blinding: Scalar)->  RistrettoPoint{
        ...
        //scalar.extend(msg);
        RistrettoPoint::multiscalar_mul(iter::once(&blinding)
                                            .chain(values.iter()), 
                                        iter::once(&self.H)
                                            .chain(self.G_vec.iter()),)
    }
...
/// 
 fn extended_pedersen_gen(capacity: usize, g: RistrettoPoint, h: RistrettoPoint) -> Vec<RistrettoPoint>{
...
/// 
 fn extended_commit(msg: &Vec<Scalar>, rscalar: Scalar, gens: &Vec<RistrettoPoint>)->  RistrettoPoint{
    let mut scalar = vec![rscalar];
    scalar.extend(msg);
    ...
    fn extended_commit_test(){
        ...
        //generate Xcomit generator points of length m+1
        let gens = extended_pedersen_gen(6, pc_gens.B, pc_gens.B_blinding);
        let shuffle_scalar: Vec<_> = cols.iter().map(|i| Scalar::from(*i as u64).clone()).collect();
